...
class TimeControlComponent extends Component {
  ...
  drawIndicator(xy1, xy2, xy3, fill, stroke, strokeWidth, callback) {
    ...
    return (<polygon points={points} style={triangleStyle} onMouseDown={() => callback(parent)} />);
  }
  drawTriangle(xy1, xy2, xy3, fill, stroke, strokeWidth, callback) {
    ...
    const parent = this;
    const points = `${xy1[0]},${xy1[1]} ${xy2[0]},${xy2[1]} ${xy3[0]},${xy3[1]}`;
    return (<polygon points={points} style={triangleStyle} onClick={() => callback(parent)} />);
  }
  ...
  rotateIndicator(parent) {
    let coordinates = [0, 100];
    coordinates = [event.pageX - 115, window.innerHeight - 102.5 - event.pageY];
  ...
  paint() {
    ...
    svgElements.push(this.drawEllipses(350, 350, 'white', '#bbbbbb', 2, this.rotateIndicator));
    svgElements.push(this.drawEllipses(320, 320, 'url(#lgrad1)', 'white', 0, this.rotateIndicator));
    svgElements.push(this.drawEllipses(290, 320, 'white', 'white', 0, this.rotateIndicator));
    svgElements.push(this.drawEllipses(250, 250, 'url(#rgrad2)', 'white', 1, () => {}));
    indicator = this.drawIndicator([xCenter - 150, yCenter], [xCenter, yCenter - 350], [xCenter + 150, yCenter], 'url(#lgrad2)', 'white', 1, this.rotateIndicator);
    ...
    return (<svg className="SvgContainer" style={svgStyle} viewBox="0 0 800 300">
      {svgElements}
    </svg>);
  }
  render() {
