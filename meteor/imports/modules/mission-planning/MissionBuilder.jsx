...
*/
MissionBuilder.prototype.getPictureWidth = () => {
 ...
 */
MissionBuilder.prototype.getPictureHeight = () => {
 ...
 */
MissionBuilder.prototype.getPictureResolution = () => {
 ...
 */
MissionBuilder.prototype.getPictureWidthWithSidelap = () => {
 ...
 */
MissionBuilder.prototype.getPictureHeightWithOverlap = () => {
 ...
 */
MissionBuilder.prototype.getPictureShootTime = () => (this.getPictureHeightWithOverlap() / this.mission.flightSpeed).toFixed(2);

/*
 Returns a JSON with the variables of the Mission
 */
MissionBuilder.prototype.getFlightVariables = () => ({
 ...
 */
MissionBuilder.prototype.calculateGrid = (/* missionSide */) => {
  ...
  for (let k = 0; k <= numberOfSegments; k += 1) {
    ...
    const projectedMidPoint = midPoint.destinationPoint(perpendicularBearing, distance * k + deltaInit);
  ...
  const Picslines = [];
  for (let k = 0; k <= numberOfSegments * 2; k++) {
    let deltaInit = 0;
    if (k == 0) { deltaInit = 0.0002; } // we move 20 cm the first line to force cutting with the other perpendiculars
    const projectedMidPoint = midPoint.destinationPoint(bearing, distance * k + deltaInit);
...
MissionBuilder.prototype.buildWaypoints = function () {
  ...
  for (i == 0; i < points.length; i++) {
    if (takePic) {
      ...
      const wpProjected = this.generateWaypoint({ lng: ptProjected.lon, lat: ptProjected.lat, altRelative: this.mission.altitude, type: wpNewPosition });
      const wpOrig = this.generateWaypoint({ lng: ptOrig.lon, lat: ptOrig.lat, altRelative: this.mission.altitude, type: wpStartPic });
      ...
      const ptProjected = ptOrig.destinationPoint(bearing, this.mission.entryMarging / 1000);
      const wpProjected = this.generateWaypoint({ lng: ptProjected.lon, lat: ptProjected.lat, altRelative: this.mission.altitude, type: wpNewPosition });
      let wpOrig = this.generateWaypoint({ lng: ptOrig.lon, lat: ptOrig.lat, altRelative: this.mission.altitude, type: wpStopPic });
      if (this.mission.type == 'lineal' && vertexCounter < numberOfVertex - 1) {
        wpOrig = this.generateWaypoint({ lng: ptOrig.lon, lat: ptOrig.lat, altRelative: this.mission.altitude, type: wpNewPosition });
      }
      wpOrig.param2 = this.mission.distPics;
      this.mission.waypoints.push(wpOrig);
